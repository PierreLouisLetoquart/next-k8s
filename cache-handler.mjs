...
class CacheHandler {
  ...
  async get(key) {
    ...
    try {
      ...
      for (const tag of cacheTags || []) {
        ...
        if (tagData && tagData.revalidatedAt > lastModified) {
          ...
          console.log(
            ...
            `♻️ Cache entry for key ${key
            } is stale due to tag ${tag} revalidation`
      ...
      if (isStale) {
        console.log(`⚠️ Cache entry for key ${key} is stale due to stale tags`);
        return null;
      }
  ...
  async set(key, data, ctx = {}) {
    ...
    try {
      ...
      console.log(`📥 Set cached data for key: ${key}`);
      if (tags.length > 0) {
  ...
  async revalidateTag(tags) {
    ...
    console.log(
      `🔄 Revalidating tags: ${tagsArray.join(', ')}`
    );
    ...
    for (const tag of tagsArray) {
      ...
      console.log(`⏰ Tag ${tag} revalidated at ${new Date(now).toISOString()}`);
    }

    console.log(`✨ Revalidation complete for tags: ${tagsArray.join(', ')}.`);
  }
}
